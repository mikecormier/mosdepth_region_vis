...
def get_plotly_shapes_from_gtf_dict(gtf_info_dict, xref="x3", yref="y3"):
    """
    ...
    Parameters:
    ----------
    1) gtf_info_dict: (dict) A dictionary of gene feature info for the current region of interests. The dictionary should be formatted like the output from
                              the 'get_gtf_region_position_info' function. (It is best to use the 'get_gtf_region_position_info' to get the correct dict)
    2) xref: (str) A string representing the plotly subplot column x position to add the output to
    2) yref: (str) A string representing the plotly subplot column y position to add the output to

    Returns:
    ++++++++
    1) shapes: (list of dicts) A list of shape dictionaries, with a shape representing a transcript or an exon
    ...
    4) y_tick_labels: (list) A list of y_tick_labels to use for a gene track. The tick labels represent transcript ids
    5) y_tick_ref_vals: (list) A list of y tick reference values, or coordinates, associated with each y_tick_label. The combination of these
    ...
    for gene_id, transcript_dict in gtf_info_dict.items():
        ...
        for transcript_id, transcript_info in transcript_dict.items():
            ...
            ## Add strand text
            for pos in (
                np.arange(transcript_info["end"] - transcript_info["start"])
                + transcript_info["start"]
            ):
                text_point_info.append(
                    [
                        pos,
                        end + 0.05,
                        ">"
            ...
            for exon_dict in transcript_info["exons"]:
                ...
                exon_point_info.append(
                    [
                        (
                            ((exon_dict["end"] - exon_dict["start"]) / 2)
                            + exon_dict["start"]
                        ),
                        end,
                        gene_id,
...
def get_shapes_for_low_coverage_cutoff(
    per_base_coverage_by_sample,
    ...
    -----------
    1) per_base_coverage_by_sample: (2d list) A 2d list, with each list representing the per base coverage values for a specific sample for the current query region
    2) region_position_list: (list) A list of positions that correlate with the per_base_coverage for each sample
...
def plot_per_base_coverage(
    ...
    region_position_list = (
        np.arange(region_dict["end"] - region_dict["start"]) + region_dict["start"]
    )
    ...
    for cov_list in per_base_coverage_by_sample:
        ...
        for (
            base_cov,
            pos,
        ) in zip(cov_list, region_position_list):
    ...
    (
        ...
        y_tick_labels,
        y_tick_ref_vals,
    ) = get_plotly_shapes_from_gtf_dict(gtf_info_dict)

    ## get low coverage shapes
    low_cov_shapes = get_shapes_for_low_coverage_cutoff(
        per_base_coverage_by_sample,
        region_position_list,
        coverage_highlight_cutoff,
        max_coverage_value,
    ...
    for i, coverage_list in enumerate(compressed_per_base_coverage_by_sample):
        fig.add_trace(
            go.Scatter(
                x=compressed_region_position_list[i],
                y=coverage_list,
                mode="lines",
    ...
    if shapes is not None:
        ...
        ## Add stranded text
        strand_text = []
        strand_indicator_interval = round(
            (region_dict["end"] - region_dict["start"]) / 20
        )
        ...
        fig.update_layout(
            yaxis3={
                ...
                "tickvals": y_tick_ref_vals,
            }
        )

    fig["layout"].update(shapes=final_shapes)
    fig.update_layout(
        title_text="Per Base Coverage Track for region {}:{}-{}".format(
            region_dict["chrom"], region_dict["start"], region_dict["end"]
        )
    )
    fig.update_layout(legend_title_text="Samples")
    fig.update_xaxes(
        title_text="{} Region Position".format(region_dict["chrom"]),
        tickformat="d",
        row=max_rows,
        col=1,
    )
...
def intersect_gtf_pr(gtf_pr, region_dict):
    """
    intersect_gtf_pr
    ================
    ...
    2) region_dict: (dict) A dictionary representing the region to intersect the pyranges object with. Required keys include: "chrom", "start", and "end"
    ...
    region_gtf_pr = gtf_pr.intersect(
        pr.from_dict(
            {
                "Chromosome": [region_dict["chrom"]],
...
def get_gtf_region_position_info(region_gtf_pr):
    ...
    Returns:
    +++++++
    1) gene_info_dict: (dict) A dictionary of gene features from the pyranges gtf object. The dict is formatted as such:
                            {key = gene_id:
                                {key = transcript_id:
...
def get_regions_from_genes(gene_list, gtf_pr):
    ...
    2) gtf_pr: (PyRanges GTF Object) A gtf/gff file loaded into a pyranges object

    Returns:
    ++++++++
    1) regions: (list) A list of a tabix styled regions based on the gene symbol coordinates in the gtf file
    """
...
def main():
    ...
    if (
        not args.region
        and not args.region_file
        and not args.gene
        and not args.gene_file
    ):
        print(
    ...
    if (
        sum(
            x is not None
            for x in [args.region, args.region_file, args.gene, args.gene_file]
        )
        > 1
    ):
        print(
            ...
            "\n\tPlease choose only one of '--region', '--gene', '--region-file', '--gene-file'".format(
                "\n\t\t".join(
                    [
                        x
                        for x in [
                            "--region" if args.region else args.region,
    ...
    if args.combine:
        ...
        output_file_path = (
            args.output if args.output.endswith(".html") else args.output + ".html"
        )
        ...
        if os.path.exists(dirname):
            ...
            print(
                "\nThe directory provided does not exists: '{}'".format(
                    dirname if dirname else "(No directory provided)"
                )
            )
            print("A temporary directory will be used: '{}'".format(temp_dir))
            output_dir = temp_dir
        ...
        basename = (
            ...
            else os.path.basename(args.output) + ".html"
        )
        print(
            "Each region html file will be written with the following convention: '<region>_{}'".format(
                basename
            )
        )
    ...
    ## Gather regions to get coverage for
    gene_list = []
    print("\nGathering Regions")
    if args.region:
        regions = args.region
        ...
        try:
            ...
            print(
                "\n!!ERROR!! Unable to open the region file '{}'. Please make sure it exists".format(
                    args.region_file
                )
            )
    elif args.gene_file:
    ...
    print(
        "\nAdding low coverage annotation based on coverage <= {}".format(
            args.lch_cutoff
        )
    )
    ...
    for i, region in enumerate(regions):
        ...
        base_div_id = "{}{}:{}-{}".format(
            gene_list[i] + ": " if gene_list[i] != "" else gene_list[i],
            region_dict["chrom"],
            region_dict["start"],
            region_dict["end"],
        )
        ...
        kde_html_string = plot_kde_per_sample(
            by_sample_coverage, sample_list, sample_colors, region_dict
        )
        ...
        (
            kde_z_score_html_string,
            z_scores_per_base_per_sample,
        ) = plot_z_score_distribution(
            by_sample_coverage, sample_list, sample_colors, region_dict
        )
        # kde_z_score_plotly_js = extract_plotly_plot_from_html(kde_z_score_html_string, base_div_id + "_3")
        kde_z_score_plotly_js = extract_plotly_plot_from_html(
            kde_z_score_html_string, "div_3"
        )
        plotly_plots += "\t\t\t" + kde_z_score_plotly_js + "\n\n"

        print("\t- Generating General Stats Table")
        table_html_string, max_coverage_value, has_low_coverage = get_plotly_table(
            by_sample_coverage, sample_list, args.lch_cutoff
        )
        ...
        if not filled_plotly_template:
            ...
            filled_plotly_template += string.Template(
                plotly_script_if_template
            ).substitute(
                region=base_div_id, low_cov=count_low_cov_shapes, pplots=plotly_plots
            )
            plotly_plots = ""
        else:
        ...
        if not args.combine:
            ...
            try:
                ...
                print(
                    "!!ERROR!! Problem loading the template html file. Please make sure it exists. Template file = '{}'".format(
                        args.tmpl
                    )
                )
                print(str(e))
            ...
            output_file_path = os.path.join(
                output_dir, "{}_{}".format(prefix, os.path.basename(args.output))
            )
